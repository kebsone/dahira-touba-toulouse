...
export class SlideshowComponent implements DoCheck {
  ...
  onSwipe(e: TouchEvent, when: string): void {
    ...
    if (indexDirection === 0) { return; } else { this.onSlide(indexDirection, true); }
  }
  ...
  private slide(indexDirection: number, isSwipe?: boolean): void {
    ...
    if (!this.slides[this.slideIndex].loaded) { this.loadRemainingSlides(); }
    if (indexDirection === 1) { this.slideRight(oldIndex, isSwipe); } else { this.slideLeft(oldIndex, isSwipe); }
  ...
  private slideLeft(oldIndex: number, isSwipe?: boolean): void {
    ...
    if (isSwipe === true) { this.onSwipeLeft.emit(this.slideIndex); } else { this.onSlideLeft.emit(this.slideIndex); }
  ...
  private slideRight(oldIndex: number, isSwipe?: boolean): void {
    ...
    if (isSwipe === true) { this.onSwipeRight.emit(this.slideIndex); } else { this.onSlideRight.emit(this.slideIndex); }
